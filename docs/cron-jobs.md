...
| `reconcile-pending-topups` | every 15 min | Settles wallet top-ups whose Safaricom callback never arrived, by asking Daraja how each push ended. Every top-up was pushed with the *order* callback URL, so its confirmation went to an endpoint that resolves the id against `Orders`, found nothing and returned 400 — a retry instruction, not an acknowledgement — while the customer's money had already left their phone. The URL is now named per caller, so this exists for the residue and for the ordinary case of Safaricom exhausting its retries against a restart. **Watch the counts.** A number that keeps coming back does not mean customers are failing to pay; it means callbacks are not being delivered, which is a defect in the deployment. It settles only what Safaricom positively resolves — an unanswerable query is not a reason to credit a wallet *or* to write a payment off — and escalates anything past `topup_reconcile_max_age_hours` to Sentry rather than deciding for itself. |
...
| `reconcile-customer-cohorts` | daily, 04:00 | Re-derives `Customer_First_Delivery` from the orders table and repairs any row that disagrees. The table is what turned the growth screen's cohort query from an aggregate over every delivered order that has ever existed into a join against one row per customer; it is written on the delivery path, and this is what makes depending on that write safe. Off-peak, and after `deposit-maintenance`, because it is pure housekeeping. **You do not have to watch this one.** A delivery path added without `record_acquisition` fails CI — `test_customer_cohorts.py` discovers every call that can move an order to `delivered` rather than listing the ones that existed when it was written — and any drift this sweep does correct raises a tagged Sentry event (`reconciliation: customer_cohorts`) carrying the counts. What is left for it to find is the residue CI cannot see: a delivery reversed after the fact, a row corrected by hand, a restore from an older backup. |
