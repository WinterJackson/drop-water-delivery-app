 ...
 *
 * Handles the four things every backend call needs and that the raw `fetch`
 * calls scattered through the hooks were each doing differently (or not at all):
 *   1. injects the Clerk JWT,
 *   2. signs the user out on a 401 instead of failing silently — in one place,
 *      rather than copy-pasted at fifteen call sites,
 *   3. converts every failure into an `ApiError` whose message is the backend's
 *      own `detail`, so callers can show it verbatim,
 *   4. applies a timeout. `fetch` has none, so a hung request hung forever and
