 ...
 *
 * The customer app has had this since launch; the rider app did not, which had it
 * exactly backwards. A customer on a stale build sees wrong prices. A rider on a
 * stale build is mid-delivery, holding somebody's water and somebody's cash — and is the person least able to
...
export async function checkForAppUpdate() {
    try {
        ...
        const data = await apiFetch<AppVersionResponse>(RiderApiRoutes.AppVersion.path, {
            kind: "read",
        });
        ...
        if (minVersion && isVersionLower(currentVersion, minVersion)) {
            Alert.alert(
                "Update Required",
                "A new version of Drop Rider is available. Please update to continue.",
                [
                    {
                        text: "Update Now",
                        onPress: () => {
