...
import { Download, ExternalLink, Mail, Calendar, ShieldCheck, TrendingUp, Layers, CheckCircle2 } from "lucide-react";
import { Button } from "@/components/ui/Button";
import { InvestorModal } from "@/components/investor/InvestorModal";

export function PitchDeckView() {
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [modalTab, setModalTab] = useState<"contact" | "schedule">("schedule");
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-8 border-b border-[var(--border)]">
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <Button 
              ...
              variant="outline" 
              onClick={openContact}
              className="gap-2 bg-[var(--surface-muted)] hover:bg-[var(--surface-raised)] border border-[var(--border)]"
            >
            ...
            <Button 
              ...
              className="gap-2 bg-[var(--foreground)] text-[var(--background)] hover:opacity-90 shadow-md"
            >
        ...
        <div className="rounded-2xl bg-[var(--surface-muted)] border border-[var(--border)] p-8 text-center sm:text-left sm:flex items-center justify-between gap-6">
          <div>
            ...
            <p className="text-sm text-[var(--foreground-muted)] mt-1">
              We welcome strategic angel investors (NaiBAN), syndicates, and ecosystem partners.
            </p>
          </div>
          <div className="mt-4 sm:mt-0 flex flex-wrap gap-3 shrink-0">
            ...
            <Button 
              size="default" 
              onClick={openSchedule}
              className="gap-2 bg-[var(--accent)] text-white hover:bg-[var(--accent)]/90 shadow-md"
            >
