import { WS_BASE_URL } from "@/API/routes/ApiRoutes";
...
const AUTH_REFRESH_INTERVAL_MS = 30_000;
  ...
  const connect = useCallback(async () => {
    ...
    try {
        ...
        if (!token || !mountedRef.current) { connectingRef.current = false; return; }
    ...
    // The close code is the only thing that distinguishes the server hanging up
    // (`1008`, with a reason) from the network dropping, and it is what showed
    // that a socket was dying on `Token expired` while the refresh acks were
    // visibly advancing — two different bugs with one symptom.
    ws.onclose = (event: { code?: number; reason?: string }) => {
      connectingRef.current = false;
      if (__DEV__ && event?.code !== 1000) {
        console.log(`WebSocket closed: ${event?.code ?? '?'} ${event?.reason ?? ''}`.trim());
      }
      stopAuthRefresh();
      stopLivenessWatch();
