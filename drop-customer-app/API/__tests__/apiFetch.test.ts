 ...
 *
 * This app had no `apiFetch` for a long time, and the gap is visible in what
 * grew in its place: three raw `fetch` calls, one of them posting avatars to
 * Cloudinary with an unsigned preset. A missing primitive is not a neutral
 * absence — callers route around it, and what they reach for is the thing with
 * no timeout, no HTTPS check and no error normalisation.
 *
